...
// Definindo o tipo para a estrutura da árvore
type ArvoreNo = {
  valor: string;
  filhos: ArvoreNo[];
};

const arvore: ArvoreNo = {
  valor: "A",
  filhos: [
    {
      valor: "B",
...
function buscaEmProfundidade(no: No): void {
...
function buscaEmLargura(raiz: No): void {
  const fila: No[] = [raiz];
  while (fila.length > 0) {
